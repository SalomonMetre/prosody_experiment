...
window.testSound = async function () {
    try {
        ...
        o.type = "sine";
        // Use setValueAtTime to ensure timing accuracy on Linux systems
...
window.start = async function () {
    ...
    // Payload matches the 'Participant' Pydantic model exactly
    const payload = {
    ...
    try {
        ...
        alert("Server error: Database initialization failed.");
        console.error(e);
    }
};
...
async function run() {
    ...
    // 300-500ms jittered Inter-Stimulus Interval
    await new Promise(r => setTimeout(r, 300 + Math.random() * 200)); 
    await play(trials[i].s2);

    // Precise color coding for instructions
...
window.choose = async function (k) {
    ...
    i++;
    setTimeout(run, 500); // 500ms post-response buffer
};
